...
const VOLATILITY = {
USD: 0.018, EUR: 0.022, XAU: 0.028,
MXN: 0.065, ETH: 0.110, BTC: 0.095,
};
const STRENGTH = {
USD: 0.95, EUR: 0.90, XAU: 0.85,
MXN: 0.40, BTC: 0.55, ETH: 0.45,
};
...
const DEFAULT_RATES = {
MXN: 17.21, USD: 1, EUR: 0.849,
BTC: 81351, ETH: 2357, XAU: 2320,
};

function buildRates(prices) {
return {
USD: 1,
MXN: prices?.exchangeRates?.MXN?.usdRate ?? DEFAULT_RATES.MXN,
EUR: prices?.exchangeRates?.EUR?.usdRate ?? DEFAULT_RATES.EUR,
BTC: prices?.prices?.BTC?.usd ?? DEFAULT_RATES.BTC,
ETH: prices?.prices?.ETH?.usd ?? DEFAULT_RATES.ETH,
XAU: prices?.prices?.XAU?.usd ?? DEFAULT_RATES.XAU,
};
}

function convert(amount, fromId, toId, rates) {
const fromUSD = fromId === “USD” ? amount : amount / rates[fromId];
const result = toId === “USD” ? fromUSD : fromUSD * rates[toId];
return result;
}

function fmt(value, id) {
if (id === “BTC”) return `₿ ${value.toFixed(6)}`;
if (id === “ETH”) return `Ξ ${value.toFixed(4)}`;
if (id === “XAU”) return `◈ ${value.toFixed(4)} oz`;
if (id === “USD”) return `$ ${value.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
if (id === “EUR”) return `€ ${value.toLocaleString("de-DE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
return `$ ${value.toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}

function generateWave(points = 140, amplitude = 0.05, bias = 0) {
const data = [];
let val = bias;
...
return { …p, top: generateWave(140, p.topAmp, p.topBias), bot: generateWave(140, p.botAmp, p.botBias) };
}, []);
...
function drawScene(ctx, cW, cH, d, alpha) {
  ...
  ctx.beginPath();
  ctx.moveTo(toX(0), mid);
  for (let i=0;i<n;i++) ctx.lineTo(toX(i), toY(d.bot[i]));
  ctx.lineTo(toX(n-1), mid);
  ctx.closePath();
  ...
  for (let i=1;i<n;i++) ctx.lineTo(toX(i), toY(d.bot[i]));
  ctx.strokeStyle = "rgba(137,175,255,0.75)";
  ctx.lineWidth = 1.6; ctx.lineJoin = "round"; ctx.stroke();

  const pulse = (Math.sin(frameRef.current * 0.07) + 1) / 2;
  const lx = toX(n-1); const ly = toY(d.top[n-1]);
  ...
  if (hoverRef.current) {
    const { x, index } = hoverRef.current;
    ctx.save();
    ctx.strokeStyle = "rgba(255,255,255,0.10)";
    ctx.lineWidth = 0.75; ctx.setLineDash([2,6]);
    ctx.beginPath(); ctx.moveTo(x,0); ctx.lineTo(x,cH); ctx.stroke();
    ctx.setLineDash([]);
    const ty = toY(d.top[index]);
    ctx.beginPath(); ctx.arc(x, ty, 3, 0, Math.PI*2);
    ctx.fillStyle = "rgba(139,124,255,0.9)"; ctx.fill();
    const by = toY(d.bot[index]);
    ctx.beginPath(); ctx.arc(x, by, 3, 0, Math.PI*2);
...
function tick() {
  ...
  if (transRef.current.active) {
    const { from, to, alpha } = transRef.current;
    drawScene(ctx, cW, cH, from, 1-alpha);
    drawScene(ctx, cW, cH, to, alpha);
...
return (
<div style={{ background: “#070707”, minHeight: “100vh”, display: “flex”, alignItems: “center”, justifyContent: “center”, fontFamily: “‘DM Mono’, monospace” }}>
<div style={{ width: 300 }}>
...
<button onClick={onCancel} style={{ marginTop: 10, width: “100%”, background: “none”, border: “none”, color: “rgba(255,255,255,0.14)”, fontFamily: “‘DM Mono’, monospace”, fontSize: 9.5, letterSpacing: “0.12em”, cursor: “pointer”, padding: “10px” }}>cancelar</button>
</div>
</div>
);
}
...
const topAmount    = 20000;
const bottomValue  = convert(topAmount, topCurrency.id, bottomCurrency.id, rates);
const bottomAmount = fmt(bottomValue, bottomCurrency.id);

function swap() {
setSwapped(true); setTimeout(() => setSwapped(false), 400);
const tmp = topCurrency; setTopCurrency(bottomCurrency); setBottomCurrency(tmp);
}
...
<CurrencySelector
which={selectingFor}
onSelect={c => { selectingFor === “top” ? setTopCurrency(c) : setBottomCurrency(c); setSelectingFor(null); }}
...
```
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px 8px" }}>
        <LensIcon size={28} pulse />
        <span style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 200, fontSize: 17, letterSpacing: "0.38em", color: "rgba(255,255,255,0.82)" }}>l e n s</span>
        ...
        {!expanded && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 20px" }}>
            <div style={{ textAlign: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 54, fontFamily: "'DM Sans',sans-serif", fontWeight: 200, color: "#fff", letterSpacing: "-0.01em", lineHeight: 1 }}>
            ...
            <button onClick={swap} style={{
              ...
              animation: swapped ? "swapSpin 0.38s ease" : "none",
            }}>⇅</button>

            <div style={{ textAlign: "center", marginTop: 8 }}>
              <div style={{ fontSize: 44, fontFamily: "'DM Sans',sans-serif", fontWeight: 200, color: "rgba(255,255,255,0.62)", letterSpacing: "-0.01em", lineHeight: 1 }}>
                {bottomAmount}
              </div>
              <div style={{ marginTop: 10 }}>
            ...
            <div onClick={() => setExpanded(true)} style={{ marginTop: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, cursor: "pointer" }}>
              ...
              <span style={{ fontSize: 7.5, letterSpacing: "0.18em", color: "rgba(139,124,255,0.32)", textTransform: "uppercase" }}>tensión relativa</span>
            </div>
          </div>
        )}
        ...
        {expanded && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "4px 16px 0" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 22, fontFamily: "'DM Sans',sans-serif", fontWeight: 200, color: "#fff" }}>
                  {topCurrency.symbol || ""}{(20000).toLocaleString("es-MX")}
                </div>
                <CurrencyPill currency={topCurrency} onClick={() => setSelectingFor("top")} />
              </div>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 22, fontFamily: "'DM Sans',sans-serif", fontWeight: 200, color: "rgba(255,255,255,0.58)" }}>{bottomAmount}</div>
            ...
            <div style={{ display: "flex", gap: 5, marginBottom: 7 }}>
              {[
                ...
                { color: "rgba(137,175,255,0.62)", label: "mayor tensión / volatilidad" },
              ].map((item, i) => (
      ...
      {["PRINCIPAL", "TENSIÓN"].map((label, i) => {
        ...
        return (
          <button key={label} onClick={() => setExpanded(i===1)} style={{
            ...
            fontFamily: "'DM Mono',monospace", fontSize: 8.5, letterSpacing: "0.16em",
...
return (
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400&family=DM+Sans:wght@200;300;400&display=swap'); *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; } body { background: #070707; } @keyframes fadeUp { from { opacity:0; transform:translateY(16px); } to { opacity:1; transform:translateY(0); } } @keyframes lensGlow { 0%,100% { box-shadow:0 0 0 0 rgba(139,124,255,0); } 50% { box-shadow:0 0 14px 2px rgba(139,124,255,0.18); } } @keyframes swapSpin { 0% { transform:rotate(0deg) scale(1); } 50% { transform:rotate(180deg) scale(0.82); } 100% { transform:rotate(360deg) scale(1); } } @keyframes breathe { 0%,100% { opacity:0.55; } 50% { opacity:1; } }`}</style>
);
}
